...
# ---------------------------------------------------------------- compact panels for the 13-page version
def compact_panels():
    """Regenerate the panels used in the page-limited manuscript at small sizes."""
    import matplotlib.pyplot as plt
    global W, H
    W0, H0 = W, H
    small = {"font.size": 7, "axes.labelsize": 7, "legend.fontsize": 5.6, "xtick.labelsize": 6, "ytick.labelsize": 6,
             "lines.markersize": 3.2, "lines.linewidth": 1.0}
    with plt.rc_context(small):
        # 3-panel rows (0.32\textwidth): sweeps A (peak, burst, dmax) and B (power, segments, tau)
        W, H = 2.25, 1.85
        fig_sweep("peak", "Peak arrival rate $h$ (packets/slot)", "c_peak", legend_loc="lower right")
        fig_sweep("burst", r"Mean burst duration $1/\alpha$ (ms)", "c_burst", xfun=lambda a: 1e3 / a, xscale="log", xticks=[0.25, 0.5, 1, 2, 4], legend_loc="lower right")
        fig_sweep("dmax", r"Latency budget $D_{\max}$ (ms)", "c_dmax", xfun=lambda v: v * 0.1, legend_loc="lower left")
        fig_sweep("power", r"Per-segment transmit power $P_{\max}$ (dBm)", "c_power", legend_loc="lower left")
        fig_sweep("segments", "Number of segments $M$ ($D_x = 40$ m)", "c_segments", legend_loc="lower left")
        fig_sweep("tau", r"PA repositioning delay $\tau_r$ (slots)", "c_tau", legend_loc="lower right")
        fig_sweep("cfg", r"Segment activation delay $\tau_{\rm cfg}$ (slots)", "c_cfg", schemes=["TLA-SWAN-cfg", "TLA-SWAN-cfg-warm", "SWAN-FULL-cfg"], legend_loc="lower right")
        # single-device panels separately (3-panel row with fixedj)
        _single_panels()
        fig_fixedj_compact()
        # 4-panel row (0.24\textwidth): energy, modes, target, cfg
        W, H = 1.72, 1.6
        fig_energy(); os.replace(os.path.join(FIG, "fig_energy.pdf"), os.path.join(FIG, "c_energy.pdf")); os.replace(os.path.join(FIG, "fig_energy.png"), os.path.join(FIG, "c_energy.png"))
        fig_modes(); os.replace(os.path.join(FIG, "fig_modes.pdf"), os.path.join(FIG, "c_modes.pdf")); os.replace(os.path.join(FIG, "fig_modes.png"), os.path.join(FIG, "c_modes.png"))
        fig_target(); os.replace(os.path.join(FIG, "fig_target.pdf"), os.path.join(FIG, "c_target.pdf")); os.replace(os.path.join(FIG, "fig_target.png"), os.path.join(FIG, "c_target.png"))
        fig_sweep("cfg", r"Activation delay $\tau_{\rm cfg}$ (slots)", "c_cfg4", schemes=["TLA-SWAN-cfg", "TLA-SWAN-cfg-warm", "SWAN-FULL-cfg"], legend_loc="lower right")
        # key figure (2 panels) a bit shorter
        W, H = 3.0, 2.0
        fig_key(); os.replace(os.path.join(FIG, "fig_key.pdf"), os.path.join(FIG, "c_key.pdf")); os.replace(os.path.join(FIG, "fig_key.png"), os.path.join(FIG, "c_key.png"))
        W, H = 3.3, 2.3
        fig_ccdf(); os.replace(os.path.join(FIG, "fig_ccdf.pdf"), os.path.join(FIG, "c_ccdf.pdf")); os.replace(os.path.join(FIG, "fig_ccdf.png"), os.path.join(FIG, "c_ccdf.png"))
        fig_hetero(); os.replace(os.path.join(FIG, "fig_hetero.pdf"), os.path.join(FIG, "c_hetero.pdf")); os.replace(os.path.join(FIG, "fig_hetero.png"), os.path.join(FIG, "c_hetero.png"))
    W, H = W0, H0
    # restore the standard-size versions of the renamed figures
    fig_energy(); fig_modes(); fig_target(); fig_key(); fig_ccdf(); fig_hetero()


def fig_fixedj_compact():
    R = load("fixedj")
    fig, ax = plt.subplots(figsize=(W, H))
    for s in ["SWAN-FIXJ-onoff", "SWAN-FIXJ-poisson"]:
        if s not in R:
        ...
        vals = sorted(R[s].keys())
        _series(ax, vals, _floor([R[s][v]["pv"] for v in vals], None, [R[s][v]["arr"] for v in vals]), s)
    ax.set_yscale("log"); ax.set_xlabel("Fixed aggregation depth $j$ (TDMA)"); ax.set_ylabel(r"$\Pr\{D>D_{\max}\}$")
...
def _single_panels():
    ...
    _series(ax, vals, _floor([R["SINGLE-SA"][v]["pv"] for v in vals], None, [R["SINGLE-SA"][v]["arr"] for v in vals]), "SINGLE-SA")
    hs = sorted(approx)
    ax.plot(hs, [np.mean(approx[h]) for h in hs], "k-.", lw=1.0, label="EB/EC approximation")
    ax.set_yscale("log"); ax.set_xlabel("Peak arrival rate $h$ (packets/slot)"); ax.set_ylabel(r"$\Pr\{D>D_{\max}\}$")
    ax.set_ylim(1e-7, 1); ax.legend(loc="lower right")
    _save(fig, "c_single_a")
    fig, ax = plt.subplots(figsize=(W, H))
    hs_all = sorted({p[3] for p in pts})
    for i, h in enumerate(hs_all):
        xs = [p[0] for p in pts if p[3] == h]; ys = [max(p[1], p[2]) for p in pts if p[3] == h]
        obs = [p[1] > 0 for p in pts if p[3] == h]
        ax.scatter([x for x, o in zip(xs, obs) if o], [y for y, o in zip(ys, obs) if o], marker="osv^D*"[i % 6], s=16, color=PALETTE[i % 8], label=f"$h={h:g}$")
        ax.scatter([x for x, o in zip(xs, obs) if not o], [y for y, o in zip(ys, obs) if not o], marker="osv^D*"[i % 6], s=16, facecolors="white", edgecolors=PALETTE[i % 8])
    lim = [1e-7, 1]
    ax.plot(lim, lim, "k--", lw=0.7); ax.plot(lim, [l * 3 for l in lim], "0.6", lw=0.5, ls=":"); ax.plot(lim, [l / 3 for l in lim], "0.6", lw=0.5, ls=":")
    ax.set_xscale("log"); ax.set_yscale("log"); ax.set_xlim(1e-7, 1); ax.set_ylim(1e-7, 1)
    ax.set_xlabel("approximation"); ax.set_ylabel("simulation (per drop)")
    ax.legend(ncol=2, loc="upper left")
    _save(fig, "c_single_b")
