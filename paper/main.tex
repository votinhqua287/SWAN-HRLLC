...
\end{equation}
the FBL rate of user $k$ in packets per slot at the nominal reliability $\varepsilon_0$ \cite{Polyanskiy2010}, with $\gamma_k^{\mathrm{SA}}(\vect x,j_0,1)$ from \eqref{eq:snr_sa}; $R_k^{\mathrm{SA}}\triangleq R_k^{(M)}$ is the full-aggregation rate. We define the \emph{tail-latency margin} of user $k$ as $\Gamma_k(\vect x)=R_k^{(j_0)}(\vect x)/\creqk{k}$: a margin below one means that even exclusive service by its $j_0$ best segments cannot meet the tail target, while a large margin leaves room for multiplexing with other users. TAPP maximizes the worst margin,
\begin{equation}\label{eq:P2}
\text{(P2)}\quad\max_{\vect x\in\mathcal X_1\times\dots\times\mathcal X_M}\ \min_{k\in\Kset}\ \frac{R_k^{(j_0)}(\vect x)}{\creqk{k}}.
\end{equation}
...
\begin{remark}
TAPP differs from rate-maximizing placement in two ways. It is max-min rather than sum, so no user is sacrificed, and the margins are normalized by $\creqk{k}$, so a critical or bursty user attracts PAs even if its channel is already good. In the homogeneous case ($\creqk{k}=\creq$ for all $k$) TAPP reduces to max-min placement of the $j_0$-segment rate; the difference becomes visible with heterogeneous classes (Section~\ref{sec:results}). The depth $j_0$ trades coverage against aggregation: $j_0=1$ places each PA above the users it serves alone, $j_0=M$ concentrates the PAs, and $j_0=2$ was found to be a robust compromise.
\end{remark}
...
\caption{per-class violation probability for six placement rules (TAS scheduling).}
\label{fig:hetero}
\end{subfigure}\hfill
\begin{subfigure}[t]{0.49\columnwidth}
\includegraphics[width=\linewidth]{fig_bcd}
\caption{convergence of the TAPP objective for five user drops.}
\label{fig:bcd}
\end{subfigure}
